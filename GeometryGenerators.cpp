...
public:
    ...
    // NOT WORKING with glu1.0 from system32 - need to compile glu on 64 bit
    void sample(vector<Vec3>* pnts) {
        gluNurbsProperty(m_nurb, GLU_NURBS_MODE, GLU_NURBS_TESSELLATOR);
        //gluNurbsCallbackData(m_nurb, pnts); // glu 1.3 needed
        g_addto = pnts;
...
public:
    static vector<Vec3>* g_addto;
};

vector<Vec3>* NurbsCurve::g_addto = NULL;

void WINAPI vertexData( GLfloat *vertex, void *userData ) {
    //vector<Vec3>* pnts = static_cast<vector<Vec3>*>(userData);
    vector<Vec3>* pnts = NurbsCurve::g_addto;
    Vec3 p(vertex, 3);
    pnts->push_back(p);
}
