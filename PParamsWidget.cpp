...
void PParamsWidget::commit()
{
    ...
    foreach(ParamUi* pui, m_paramUi)
    {
        ParamInput pi(pui->name->text(), 
            ...
            pui->value->text(), pui->m_dest, m_pass);
...
{
    ParamInput curpi(pui->name->text(), pui->m_dtype, pui->value->text(), pui->m_dest, NULL);
    ...
    if (!curpi.name.isEmpty() && !curpi.value.isEmpty())
    {
        if (pui->m_dest == EDModel) {
            if (!m_doc->parseSingleParam(curpi, false))
                c = ERROR_COL;
            ...
            if (pui->index < m_pass->params.size())
            {
                ...
                if (progpi.name == curpi.name && getBaseType(progpi.type) == getBaseType(curpi.type) && progpi.dest == curpi.dest)
                {
...
{
    addParam(ParamInput("", EPFloat, "", EDUniform, NULL));
}
...
void PParamsWidget::addParam(const ParamInput& pi) //const QString& name, EParamType type, const QString& value)
{
    ...
    pui->uniAttr->setToolTip("Uniform/Attribute/Model");
