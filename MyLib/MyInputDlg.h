...
class WidgetIn : public QObject
{
    Q_OBJECT
public:
    WidgetIn(Prop* v, QWidget* w) :pv(v), pw(w), inCommit(false), inCommit2(false)
    ...
    virtual void commit2Imp() {}

    bool isInCommit() const { return inCommit; }

    enum EFlags { None = 0, DoModal = 1, WithAlpha = 2 }; // for ColorSelIn (DoModal is infact DoModalLess)
signals:
    void commitedChange();
...
public slots:
    ...
    void commit2() // another widget that controls the value (SliderIn)
    {
        inCommit2 = true;
        commit2Imp();
        inCommit2 = false;
...
{
public:
    SliderIn(TypeProp<T>* _v, QSlider *_w, QLabel* _numl, T _mult = 1, bool autoup = true, QLineEdit* _numed = NULL) 
        :WidgetIn(_v, _w), v(_v), w(_w), numl(_numl), mult(_mult), numed(_numed)
    {
        widgetUpdate();
        if (autoup) {
        ...
        if (numed && autoup) {
            connect(numed, SIGNAL(textEdited(const QString&)), this, SLOT(commit2()));
        }
    }
    virtual void commitImp()
    {
    ...
    virtual void residualUpdate() 
    {
        ...
        if (numed != NULL && !inCommit2)
            numed->setText(toString(w->value() / mult));
    }
    ...
    virtual void commit2Imp()
    {
        auto tv = qvariant_cast<T>(fromString(numed->text(), typeid(T)));
        *v = tv;
    }
...
struct ImageLabelIn : public WidgetIn
{
public:
    ImageLabelIn(TypeProp<QImage>* _v, QLabel *_w, bool autoup = true) 
        :WidgetIn(_v, _w), v(_v), w(_w)
    {
    ...
    virtual void reloadImp() {
        if (v->val().isNull())
            return;
        w->setPixmap(QPixmap::fromImage(v->val()).scaled(w->width()-2, w->height()-2 ));
    }
...
struct ColorSelIn : public WidgetIn
{
    Q_OBJECT
public:
    // flags is WidgetIn::EFlags
    ColorSelIn(TypeProp<QColor> *_v, QAbstractButton *_bw, int flags = None, bool _autoup = true);
...
public:
    ...
    // a line of widgets -  a few widgets side by side
